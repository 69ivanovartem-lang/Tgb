...
def create_enhanced_graph_visualization(notes, graph):
    """Создает улучшенную визуализацию графа с правильным расположением узлов"""
    try:
        # Создаем граф
        G = nx.Graph()
        
        # Добавляем узлы и считаем количество связей для каждого
        node_degrees = {}
        for note_id, title in notes.items():
            short_title = title[:15] + '...' if len(title) > 15 else title
            G.add_node(note_id, label=f"{note_id}: {short_title}")
            # Количество связей = степень узла в графе
        ...
        # Разделяем узлы на связанные и несвязанные
        ...
        print(f"🔗 Связанных узлов: {len(connected_nodes)}")
        print(f"🔴 Изолированных узлов: {len(isolated_nodes)}")
        
        # Создаем кастомное расположение
        pos = {}
        
        # Располагаем связанные узлы СПРАВА в виде компактной сети
        if connected_nodes:
            # Создаем подграф только из связанных узлов
            connected_subgraph = G.subgraph(connected_nodes)
            
            # Используем spring layout с меньшим k для более компактного расположения
            connected_pos = nx.spring_layout(connected_subgraph, k=0.8, iterations=100, scale=2)
            
            # Сдвигаем все связанные узлы в правую часть и сжимаем их
            for node, (x, y) in connected_pos.items():
                # Нормализуем координаты и сдвигаем вправо
                pos[node] = (x * 0.8 + 3.5, y * 0.8)  # Более компактно и правее
        
        # Располагаем несвязанные узлы СЛЕВА в аккуратной сетке
        if isolated_nodes:
            num_isolated = len(isolated_nodes)
            
            # Создаем сетку с оптимальным количеством столбцов
            if num_isolated <= 10:
                cols = 2
            elif num_isolated <= 20:
                cols = 3
            else:
                cols = 4
                
            rows = int(np.ceil(num_isolated / cols))
            
            # Центрируем сетку по вертикали
            start_y = (rows - 1) * 0.8 / 2
            
            for i, node in enumerate(isolated_nodes):
                row = i // cols
                col = i % cols
                # Располагаем слева в аккуратной сетке
                pos[node] = (-3.5 + col * 1.2, start_y - row * 0.8)
        
        # Настраиваем визуализацию
        plt.figure(figsize=(16, 10))
        
        # Рисуем связанные узлы (СПРАВА) - КОМПАКТНО
        if connected_nodes:
            # Размер узла зависит от количества связей
            connected_sizes = [400 + node_degrees[node] * 150 for node in connected_nodes]
            connected_sizes = [min(size, 2500) for size in connected_sizes]
        ...
        # Рисуем несвязанные узлы (СЛЕВА) - АККУРАТНАЯ СЕТКА
        if isolated_nodes:
        ...
        # Рисуем связи (только для связанных узлов)
        if connected_nodes:
        ...
        # Подписи узлов - пронумерованы по порядку
        labels = {}
        node_order = sorted(notes.keys())  # Сортируем по ID
        
        for i, node_id in enumerate(node_order, 1):
            labels[node_id] = str(i)  # Просто номер по порядку
        
        # Рисуем подписи с улучшенным позиционированием
        nx.draw_networkx_labels(G, pos, labels, font_size=9, font_weight='bold',
                               font_color='black', verticalalignment='center')
        
        # Добавляем легенду с реальными названиями
        legend_text = "📋 Легенда (номера соответствуют узлам на графе):\n\n"
        ...
        for i, node_id in enumerate(node_order, 1):
            ...
            if node_id in connected_nodes:
                connections = node_degrees[node_id]
                connected_titles.append(f"{i}: {short_title} (🔗 {connections})")
            else:
                isolated_titles.append(f"{i}: {short_title}")
        
        # Сортируем связанные заметки по количеству связей (убывание)
        connected_titles.sort(key=lambda x: int(x.split('(🔗 ')[1].split(')')[0]), reverse=True)
        
        if connected_titles:
            legend_text += "🔵 **СВЯЗАННЫЕ ЗАМЕТКИ** (справа):\n" + "\n".join(connected_titles[:12])
            if len(connected_titles) > 12:
                legend_text += f"\n... и еще {len(connected_titles) - 12} связанных заметок"
        
        if isolated_titles:
            legend_text += "\n\n🔴 **ИЗОЛИРОВАННЫЕ ЗАМЕТКИ** (слева):\n" + "\n".join(isolated_titles[:8])
            if len(isolated_titles) > 8:
                legend_text += f"\n... и еще {len(isolated_titles) - 8} изолированных заметок"
        
        # Статистика
        total_connections = sum(node_degrees.values()) // 2  # Каждая связь учтена дважды
        legend_text += f"\n\n📊 **СТАТИСТИКА**:"
        legend_text += f"\n• Всего заметок: {len(notes)}"
        legend_text += f"\n• Связанных: {len(connected_nodes)}"
        legend_text += f"\n• Изолированных: {len(isolated_nodes)}"
        legend_text += f"\n• Всего связей: {total_connections}"
        
        if connected_nodes:
            max_connections = max(node_degrees[node] for node in connected_nodes)
            legend_text += f"\n• Макс. связей у узла: {max_connections}"
        
        # Размещаем легенду
        plt.figtext(0.02, 0.02, legend_text, fontsize=7, 
                   bbox=dict(boxstyle="round,pad=0.5", facecolor="white", alpha=0.95))
        
        # Разделительная линия между областями
        plt.axvline(x=0, color='black', linestyle='-', alpha=0.4, linewidth=2)
        
        # Подписи областей
        plt.text(-3, 2.5, '🔴 Изолированные\nзаметки', fontsize=11, 
                bbox=dict(boxstyle="round,pad=0.4", facecolor="lightcoral", alpha=0.8),
                ha='center', weight='bold')
        plt.text(3.5, 2.5, '🔵 Связанные\nзаметки', fontsize=11,
                bbox=dict(boxstyle="round,pad=0.4", facecolor="lightblue", alpha=0.8),
                ha='center', weight='bold')
        
        # Заголовок с пояснением
        plt.title("🌳 Граф заметок Zettelkasten\n(Размер узла = количество связей | Цифры = порядковые номера)", 
        ...
        logger.error(f"Ошибка при создании улучшенного графа: {e}")
        import traceback
        ...
        if not notes:
            ...
            if message_id:
                bot.edit_message_text(
                    ...
                    chat_id,
                    message_id,
                    reply_mup=create_main_keyboard()
                )
            else:
                bot.send_message(
                    chat_id,
...
🔹 Визуализация графа:
• 🔵 Справа: Связанные заметки (размер узла = количество связей)
• 🔴 Слева: Изолированные заметки  
• 🔢 Цифры: Порядковые номера заметок
• 📏 Размер: Чем больше связей - тем больше узел
...
def handle_callback(call):
    ...
    try:
        if data.startswith("view_note_"):
            ...
            bot.send_message(chat_id, "🖼️ Создаю граф с улучшенной визуализацией...")
            ...
            if graph_bytes:
                bot.send_photo(
                    ...
                    caption="🖼️ **Улучшенный граф ваших заметок**\n\n"
                           "• 🔵 **Справа**: Связанные заметки (размер = количество связей)\n"
                           "• 🔴 **Слева**: Изолированные заметки\n" 
                           "• 🔢 **Цифры**: Номера заметок по порядку\n"
                           "• 📏 **Размер узла**: Чем больше связей - тем больше кружок\n\n"
                           "📋 В легенде показаны все названия с соответствующими номерами.",
                    parse_mode='Markdown',
                    reply_markup=create_main_keyboard()
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        import networkx as nx
        import numpy as np
        print("✅ Matplotlib, NetworkX и NumPy доступны для улучшенной визуализации")
    except ImportError as e:
        print(f"❌ Библиотеки для визуализации не установлены: {e}")
        print("Установите: pip install matplotlib networkx numpy")
