...
def create_main_keyboard():
    ...
    keyboard.add(
        ...
        KeyboardButton("🌳 Дерево заметок"),  # Новая кнопка
...
def create_commands_keyboard():
    ...
    keyboard.add(
        ...
        KeyboardButton("/tree"),  # Новая команда
        KeyboardButton("/help"),
...
/tree - Визуализация дерева заметок
/help - Помощь
...
/tree - Визуализация дерева связей заметок
...
5. Просматривайте дерево связей для поиска новых идей
...
def tree_command(message):
    """Обработчик команды /tree для визуализации дерева заметок"""
    try:
...
@bot.message_handler(
    func=lambda message: message.text in ["📝 Новая заметка", "📚 Мои заметки", "🔍 Поиск", "🌳 Дерево заметок",
                                          "ℹ️ Помощь", "⚡ Все команды", "📋 Главное меню", "❌ Отмена"])
